...
async function ExportData(ViewerID, Index, Persist) {
	let Account = {
		'ViewerID': ViewerID,
		'PersistID': crypto.randomBytes(32).toString("hex")
	}
	const IsExists = await Fluoresce.Exists("OrchisAccount", Account['ViewerID']);
	if (IsExists == true) {
		Account = await Fluoresce.Read("OrchisAccount", ViewerID);
	}
	await Fluoresce.Write("OrchisAccount", ViewerID, Account);
	await Fluoresce.Write("OrchisIndex", Account['ViewerID'], Index);
	await Fluoresce.Write("OrchisPersist", Account['PersistID'], Persist);
	console.log("Wrote " + ViewerID);
}
...
async function Main() {
	...
	for (const x in SupportList) {
		const Helper = SupportList[x];
		const AccountID = 11 + SupportData['support_user_detail_list'].length;
		let CharacterID = 0;
		let DragonID = 0;
		let WeaponID = 0;
		let CrestList1 = [];
		...
		await ExportData(AccountID, Index, Persist);
	}
	fs.writeFileSync('../Library/Event/Support.json', JSON.stringify(SupportData, null, 2));
}
